  ...
  const fetchMatches = async () => {
    try {
      ...
      if (authMode === 'offline' && baseSession) {
        ...
        // Load liked lawyers as chatbot matches for guest/base users
        const likedProfiles = JSON.parse(localStorage.getItem('likedProfiles') || '[]');
        const lawyerProfiles = likedProfiles.filter((profile: any) => profile.userType === 'lawyer');
        
        const mockMatches = lawyerProfiles.map((lawyer: any) => ({
          id: `chat-${lawyer.id}`,
          client_id: offlineSession.user.id,
          lawyer_id: lawyer.id,
          ...
          last_message: {
            content: "Start a conversation with this lawyer",
            created_at: new Date().toISOString()
          }
        }));
      ...
      if (!user) {
        // Also check for guest mode without base account
        const likedProfiles = JSON.parse(localStorage.getItem('likedProfiles') || '[]');
        const lawyerProfiles = likedProfiles.filter((profile: any) => profile.userType === 'lawyer');
