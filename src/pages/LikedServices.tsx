...
import { MapPin, Star, Shield, Clock, Heart, Trash2, ArrowLeft, Eye, MessageCircle } from "lucide-react";
...
const LikedServices = ({ onBack, userType = "client" }: LikedServicesProps) => {
  ...
  const handleChatWithLawyer = (profile: MockProfile) => {
    if (profile.userType === 'lawyer') {
      // Navigate to chat with this lawyer
      navigate(`/chat/chat-${profile.id}`);
    } else {
      toast.info("Chat is only available with lawyers");
    }
  };
